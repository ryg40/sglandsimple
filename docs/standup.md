...
- The `/standup` layout (Stage 27) keeps the **chat panel in the right-hand column** and the working cards (Jira Explorer, configuration/trace, Templates, Epics, Approvals viewport) in the main left section. The chat header has a **Widen/Collapse** toggle: widening collapses the grid to a single column and expands chat to the full viewport width for easier screen-share reading; collapsing returns it to the `23rem` rail. Chat send, presence, and Summarize behave identically in both states.
