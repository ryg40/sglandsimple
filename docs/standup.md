...
- **Session history read** (`GET /api/standup/sessions`) and **snapshot read** (`GET /api/standup/sessions/{id}/snapshot`): require any authenticated user (401 otherwise).
- **Approve / reject / edit / submit approved to implementation**: gated on `canApproveStandupActions` or the `STANDUP_APPROVER_EMAILS` auth-system allowlist. The default named production approver is `simone.patel@lanGarland.com` (case-insensitive). Non-approvers receive a `forbidden` websocket error and the approval controls are disabled with an explanatory tooltip in the UI.
...
- On `xl` screens the chat column is **sticky to the viewport** (Stage 33): it tracks the window height (`sticky top-5 h-[calc(100vh-2.5rem)]`) so the **chat text input stays pinned to the bottom of the window without scrolling the page**, while the message history scrolls inside the card. This keeps attendees looking at the main viewport with the scrum master while remaining able to type at any time. The Widen/Collapse toggle is unaffected.
- The chat header now has explicit **New session**, **Previous**, and **Next** controls (Stage 35). They use the persisted standup session store (`/data/auth/standup_sessions.json` via `GET /api/standup/sessions`) so admins/approvers can reset to a fresh chat or move through old chat histories for reference. Switching sessions reconnects the websocket and refreshes messages, proposals, agent runs, trace, and approval state without a full page reload. The top session selector remains available for direct jumps.
...
- **Incoming tickets** (Stage 31) is a conditional card below Epics/Templates. It is absent when there is no intake and otherwise reads `/api/standup/incoming`, which proxies the read-only `standup_incoming_tickets` MCP tool. Each ticket shows key/summary/reporter, extracted AWS/RDS/region/team/user/email/DL entities, resolved Stage-32 directory/team context when available (reporter title, manager, teams), workflow match confidence/rationale against existing standup templates, connector-hub source badges, and a dry-run kickoff affordance. This path never assigns or writes; follow-up work still goes through the existing proposal/approval gates.
- The **Approvals viewport** is a full-width live card below the Epics/Templates/incoming-intake area, driven by `proposal.created`/`proposal.updated`/`agent.summary` websocket events. It opens with an inline **How to use this** guide (capture chat → Summarize → review/edit payload → Save → Submit/Reject) so first-time approvers understand the dry-run-by-default flow. Approvers can edit each staged JSON payload, **Save** edits without applying, **Submit** through the gated production path, or Reject. Non-approvers see the controls disabled with a tooltip and a read-only notice. A `Summarize` button triggers `agent.summarize` over the live socket. Approvals record `actor`, `decided_at`, original payload, edited payload, validation result, dry-run/live mode, and the apply result.
- The Approvals viewport now defaults to the **active approver queue** (open `proposed` items plus approved items not yet submitted to implementation) and hides already submitted/implemented/blocked/failed history unless **Show history** is enabled. Approvers get a batch **Submit approved to implementation** button that reuses the existing apply path for every approved item in the selected session and persists per-item `implementation_status`, submit actor/timestamp, and implementation result. Items leave the active queue after submission but stay recoverable in the JSON audit history.
