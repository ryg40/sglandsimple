...
- The `/standup` layout keeps the **chat panel at the top of the right-hand column** so live note capture and summarize actions stay above the approval workflow during screen-sharing.
- **Epics** remains a collapsed-by-default reference card in the right-hand column under chat. It expands into a read-only active-epic list sourced from `/api/standup/epics` (the `epics` collection via MCP). Rows show key/title/program area/status/priority, tags, regulation refs, database/platform combos, ticket refs, finding links, and Jira deep links. Selecting a row sets local selected-epic context as the seam for future agent resolution.
