...
| 21 (Deep Agent platform) | pi agent (core) + Claude Code (close-out) | main tree + `wt-s21-deploy` | **COMPLETE (pending merge)** — core tasks (arch/upgrade/profile/context/orch/runtime/hitl/agent/extend/ui) done on main by pi agent. Claude Code closed the final 7 in worktree `wt-s21-deploy` (branch `s21-deploy-obs-sec`): obs.1, security.1, bedrock.1, deploy.1 (sidecar `/healthz`), deploy.2 (`deploy/k8s/`), verify.1 (`scripts/smoke_deep_agent_platform.py`), verify.2 (restart drill). New files + additive edits to `runtime.py`/`server.py`/`web/main.py`/`compose.yaml` (sandbox block only)/`.env.example`. NOT yet committed/merged. Live-verified except LLM-routing steps (both inference endpoints were down at run time). |
