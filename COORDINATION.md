...
| 21 (Deep Agent platform) | pi agent | main tree | **IN PROGRESS** — DONE & on main: arch.1, upgrade.1 (LangChain 1.x), profile.1 (`profiles.yaml`+loader), context.1, orch.1 (`build_orchestrator`+allowlist), runtime.1 (non-blocking pollable `agent_run_*` API + `/api/agents/*` proxies), ui.1 (`/agents` operations page). Next: hitl.1, then baseline agent smokes. |
