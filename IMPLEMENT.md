...
- [ ] **S21.context.1 — Context packs for the system agents**
  ...
  - Done when: each agent loads only its compact, versioned context pack (templates, schemas, examples, runbook links) — sourced from existing material — mapped onto the subagent `skills`/`system_prompt`; no unrelated docs leak into a subagent's context. **If the Stage-24 `standup_templates` store exists, the `atlassian_agent`/`standup_agent` Jira/Confluence prompt packs read from it (the shared source of truth — see `S24.templates.api.1`) rather than a duplicated copy.**
