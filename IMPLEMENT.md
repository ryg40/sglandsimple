...
- [x] **S34.codesplit.1 — Lazy-load routes + split vendor chunks** ✅ DONE
  ...
  - Done: every route except the eager landing `Overview` is `React.lazy()` behind a `<Suspense>` fallback, so a fresh session downloads the shell + landing route instead of all routes. `vite.config.ts` `manualChunks` splits `vendor-react`, `vendor-flow` (`@xyflow/react`, ~54KB gz — only on `/architecture`), `vendor-charts` (`recharts`, ~109KB gz), `vendor-markdown` (`react-markdown`/`rehype-highlight`/`highlight.js`/`remark-gfm`, ~102KB gz — only on `/docs`,`/chat`), and `vendor-query`. Overview's single AreaChart is extracted to a lazy `overview-trend-chart` so recharts no longer blocks the landing paint. The always-mounted `GlobalAssistant` floating widget is also lazy-loaded (it pulled `react-markdown`) so `vendor-markdown` leaves the cold path entirely. Result: the old single `index-*.js` (~1.7MB / ~500KB gz) is replaced by an entry `index` chunk of **~50KB gz** plus on-demand route/vendor chunks, and the entry HTML no longer modulepreloads `vendor-markdown`/`vendor-charts`; the build's chunk-size warning is gone. Behavior unchanged; `cd web && npm run build` passes.
...
## Stage 35 — Standup approver submit queue + chat history navigation (planned)

**Goal:** Clean up the `/standup` approver experience so approved captured items do not permanently litter the active view, and give admins/approvers fast session-level context switching. The approver needs a clear **Submit approved to implementation** action that sends all approved/captured items in the current session through the existing implementation/apply path, records the result, and moves completed items out of the active approvals queue. The standup chat also needs explicit **New session / reset**, **Previous**, and **Next** history controls backed by the existing JSON session store (`/data/auth/standup_sessions.json`) so an admin/approver can quickly recall old chats for reference without losing the current session.

> **Relationship to Stages 28/29.** S28 starts the multi-session store/list plumbing; S29 adds proposal lifecycle and broader apply support. This stage is the admin/approver UX that ties those pieces together: batch-submit approved items, hide implemented/archived items by default, and add direct previous/next chat-history navigation in addition to any dropdown switcher.

### Task checklist — Stage 35

- [x] **S35.standup-approver-queue.1 — Submit approved captured items + reset/previous/next chat sessions** ✅ DONE
  - Files: `web/standup_store.py`, `web/standup_ws.py`, `web/src/components/standup-chat.tsx`, `web/src/routes/standup.tsx`, `web/src/lib/queries.ts`, `web/src/lib/types.ts`, `scripts/smoke_standup_ws.py`, `docs/standup.md`, `CHANGELOG.md`, `IMPLEMENT.md`, `progress.md`.
  - Done: `StandupStore.list_sessions()` plus authenticated `GET /api/standup/sessions` expose persisted chat-history metadata ordered by `updated_at` (message/proposal/active/implemented counts). `/standup` now owns the selected `sessionId`, renders a top session selector, and passes navigation into `StandupChat`; the chat header has **New session**, **Previous**, and **Next** controls. Switching sessions reconnects the websocket and resets local drafts/trace before the new snapshot populates messages/proposals/agent runs; `daily-standup` remains the default.
  - Done: added approver-gated websocket `proposal.submit_approved` for batch submission of approved, not-yet-submitted proposals. Each item reuses `_apply_proposal_submit()` and persists `implementation_status` (`implemented`/`blocked`/`failed`/`submitted`), `submitted_by`, `submitted_at`, `implementation_result`, and append-only `implementation_history` via `mark_proposal_submitted()`; no proposal/session data is hard-deleted. The Approvals viewport defaults to active work only and hides submitted/implemented history unless **Show history** is enabled. Non-approvers get the same disabled tooltip/`forbidden` websocket behavior.
  - Verification: `python3 -m py_compile web/*.py scripts/smoke_standup_ws.py` passes; `cd web && npm run build` passes. `scripts/smoke_standup_ws.py` now covers non-approver batch-submit rejection, approver batch-submit result persistence, and authenticated session-list history.
  - Git handoff used/documented: isolated worktree from `origin/main` (`../wt-standup-approver-sessions`, branch `standup-approver-sessions-task`), hooks installed/verified (`core.hooksPath=scripts/git-hooks`), stage by explicit path only (`git add web/standup_store.py web/standup_ws.py web/src/components/standup-chat.tsx web/src/routes/standup.tsx web/src/lib/queries.ts web/src/lib/types.ts scripts/smoke_standup_ws.py docs/standup.md CHANGELOG.md IMPLEMENT.md progress.md`), inspect `git status --short` + `git diff --cached --stat`, commit `feat(S35): standup approver submit queue + chat history navigation`, push branch, merge by PR/fast-forward only after review/smokes/build pass.
  - Depends on: S25.approver.1 (builds on S28/S29 ideas without requiring their open tasks to land first).
