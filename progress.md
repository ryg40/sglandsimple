...
2. **Stage 14 agent** — finish `S14.agent.1`: wrap `mcp/docs_agent.py` in a checkpointed LangGraph `StateGraph` with interrupt/resume at the apply gate. The `/api/docs/agent` proxy and UI hook already exist.
