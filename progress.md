...
## Session 2026-05-29 — Stage 21 deploy/obs/security/bedrock/verify (close-out)
- Closed the remaining 7 S21 tasks in worktree `wt-s21-deploy` (branch `s21-deploy-obs-sec`), branched from HEAD to avoid the other agent's main-tree WIP (uncommitted compose.yaml/CHANGELOG edits left untouched).
- **S21.obs.1 / S21.security.1 DONE** — new `mcp/deep_agent/observability.py` (metrics counters + per-profile latency, structured `[deep_agent.*]` logs, secret redaction, Mongo-backed policy/approval audit trail). Wired into `runtime.py` dispatch + run lifecycle + resume, exposed via `agent_metrics` MCP tool + `/api/agents/metrics` proxy (`?format=prometheus`). Verified live in-container: fail-closed boundary records ring+Mongo audit, redaction masks key+token-shaped values, metrics snapshot returns all keys.
- **S21.bedrock.1 DONE** — new `mcp/deep_agent/provider.py` provider switch; `runtime.py` builds models via `chat_model_for_role`. Default → `ChatOpenAI` (unchanged); `*_PROVIDER=bedrock` → `ChatBedrockConverse` (optional deps; clear `ProviderConfigError` stub when absent). Bedrock-id heuristic rejects version-dotted OpenAI names. Verified live.
- **S21.deploy.1 DONE** — `sandbox-runtime/sidecar.py` stdlib `/healthz`:8090 + baked HEALTHCHECK + compose healthcheck. Built + ran the sandbox image; transitions to `healthy`.
- **S21.deploy.2 DONE** — `deploy/k8s/` blueprint (8 manifests + README), `docs/deployment.md`. ECS/Fargate documented as alternative. All YAML parses; `deploy/k8s/secret.yaml` gitignored.
- **S21.verify.1 DONE** — `scripts/smoke_deep_agent_platform.py`. Live-passed: roster/write-tool-uniqueness, metrics, persistence, fail-closed, audit, redaction.
- **S21.verify.2 DONE** — `docs/deployment.md` restart drill; live-proved a synthetic `waiting_approval` run survived `docker compose restart mcp`.
- **Infra note:** the LLM-routing / HITL-pause / Mongo-answer smoke steps could not run live — BOTH remote inference endpoints (`:8001`, `:9292`) were down at run time (confirmed by direct curl), independent of this code. Re-run the platform smoke with `RUN_GRAPH_AGENTS=1` once an inference backend is serving.
- Validation: `python3 -m py_compile` clean on all changed `.py`; `docker build ./mcp` + `./sandbox-runtime` clean; recreated mcp from the freshly-tagged image (healthy); all live checks above green.
